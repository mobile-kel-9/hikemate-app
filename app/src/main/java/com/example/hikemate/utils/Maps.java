...
public class Maps extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private boolean isCardCollapsed = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Referensi ke bottom card dan tombol close
        View bottomCard = findViewById(R.id.bottom_card);
        ImageView btnCloseBottomCard = findViewById(R.id.btn_close_bcard);
        bottomCard.post(() -> {
            collapsedTranslationY = bottomCard.getHeight() / 2f;
        });

        btnCloseBottomCard.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (isCardCollapsed) {
                    // Expand bottom card (tampilkan sepenuhnya)
                    bottomCard.animate()
                            .translationY(0) // Kembali ke posisi awal
                            .setDuration(200)
                            .start();
                } else {
                    // Collapse bottom card (sembunyikan sebagian)
                    bottomCard.animate()
                            .translationY(collapsedTranslationY) // Geser ke bawah sebagian
                            .setDuration(200)
                            .start();
                }
...
// Memeriksa izin lokasi
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED
            ...
            ActivityCompat.requestPermissions(
                    ...
                    LOCATION_PERMISSION_REQUEST_CODE // Konstanta untuk permintaan izin
            );
...
// Mendapatkan lokasi terakhir
                ...
                .addOnSuccessListener(this, new OnSuccessListener<Location>() {
                    ...
                    public void onSuccess(Location location) {
                        if (location != null) {
                            ...
                            // Lakukan sesuatu dengan lokasi, misalnya tampilkan di log
                            Log.d("Location", "Latitude: " + latitude + ", Longitude: " + longitude);
                        } else {
                            // Lokasi terakhir tidak tersedia
                            Log.w("Location", "Lokasi terakhir tidak tersedia");
                        }
                    }
                });
    ...
    private void animateBottomCard(View card, float fromYDelta, float toYDelta) {
        TranslateAnimation animation = new TranslateAnimation(
                0, 0, // fromXDelta, toXDelta (tidak ada pergerakan horizontal)
                fromYDelta, toYDelta // fromYDelta, toYDelta (vertikal)
        );
        animation.setDuration(150); // Durasi animasi (ms)
        animation.setFillAfter(true); // Tetap di posisi akhir setelah animasi
    ...
    private void drawRoute() {
        ...
        // Tambahkan kembali marker lokasi awal dan tujuan
        mMap.addMarker(new MarkerOptions().position(lokasiAwal).title("Lokasi Anda"));
        mMap.addMarker(new MarkerOptions().position(lokasiTujuan).title("Lokasi Tujuan"));
        ...
        // Hitung jarak dan estimasi waktu berdasarkan lokasi awal terbaru
        double distance = calculateDistance(lokasiAwal, lokasiTujuan);
        ...
        if (timeRemainingTextView != null) {
            String waktuEstimasi = estimasiWaktuMenit + " menit tersisa";
